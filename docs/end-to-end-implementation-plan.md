...
- Audit je rozsireny i na hlavni desktopove Figma pruchody `/catalog/virivky/`, `/product/timberwolf/` a `/kontakt/`. Kontroluje souradnice sekci, produktu, konfiguraci, rychleho kontaktu, mapy, kontaktu a footeru podle Figma passu; u Timberwolfu navic overuje lazy-load zdroj Figma hero a konfiguraci `detail-timberwolf-*`.
