...
Disable PhotoSwipe
add_filter( 'forqy_pswp', '__return_false' );

Disable content images PhotoSwipe
add_filter( 'forqy_pswp_content', '__return_false' );
*/
...
if ( !function_exists( 'forqy_photoswipe_body_class' ) ) {
	...
	function forqy_photoswipe_body_class( $classes ) {
		...
		if ( apply_filters( 'forqy_pswp', '__return_true' ) ) {
			$classes[] = 'image--photoswipe';
		}
		if ( apply_filters( 'forqy_pswp_content', '__return_true' ) ) {
...
if ( !function_exists( 'forqy_photoswipe_content_link_attributes' ) ) {
	...
	function forqy_photoswipe_content_link_attributes( $content ) {
		...
		if ( !is_admin() && apply_filters( 'forqy_pswp_content', '__return_true' ) ) {
...
if ( !function_exists( 'forqy_photoswipe_content_link_attributes__callback' ) ) {
	...
	function forqy_photoswipe_content_link_attributes__callback( $matches ) {
		...
		return sprintf( '<a href="%1$s" data-pswp-width="%2$s" data-pswp-height="%3$s" data-cropped="true" target="_blank" %4$s>', $href, $width, $height, $attributes_n );
